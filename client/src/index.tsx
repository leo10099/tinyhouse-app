...
const App = () => {
  ...
  useEffect(() => {
    // Only try log in on first mount
    logInRef.current();
  }, []);

  // DidRequestGoogleInfo means that the login process ended succesfully or failed
  if (!viewer.didRequestGoogleInfo && !error) {
    return (
      <Layout className="app-skeleton">
        <AppHeaderSkeleton>
          <div className="app-skeleton__spin-section">
            <Spin size="large" tip="Launching Tinyhouse..."></Spin>
          </div>
        </AppHeaderSkeleton>
      </Layout>
    );
